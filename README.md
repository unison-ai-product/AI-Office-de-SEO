...
- **二層課金**: システム利用料（プラン4種: Entry/Standard/Premium/Enterprise）＋クレジット（月額パック・追加購入・予約/確定/返還のappend-only台帳・Stripe連携）
...
- `REQ-DUR-09`（AWS配置と実行移管性）: 不変イメージ（シークレット非焼き込み・設定注入）を初期からAWSへ配置し、最小構成→task/worker/DB/tenant shardを**同一イメージ・境界API不変で段階拡張**する。状態はPG/S3 Contract/queueへ外部化し、AWS固有API依存をAdapterへ隔離、stagingで復元・配置切替を演習する。
...
- `REQ-DUR-06`新設: テナント資源プロファイル→**AWS実行単位／worker pool当たり健全テナント数**（利用率・キュー遅延で定義し負荷試験で実測）→最小構成からworker、DB、tenant shardを境界API不変で段階拡張→**基盤費÷健全テナント密度＝1社当たり基盤原価**をCost Tableへ配賦する。
