...
> **監査時点スナップショット**: 本書の「欠落」「残件」「未完成」は各指摘を発見した時点の証拠であり、後続コミットで解消された項目を現在も未完了と判定する正本ではない。現行状態はArtifact Alignment Ledger、Open Items Register、manifest、要求監査結果を参照する。プロトタイプ未追随の項目だけはPrototype Modernization Registerを正本とする。
