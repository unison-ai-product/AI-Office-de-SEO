...
> **履歴スナップショット**: 本書は2026-07-09時点の旧プロトタイプ実装を調べた記録である。本文中の「正本」「現在」「実装・検収済み」は当時のコードとタブ構成に限り、現行要求の正本を意味しない。現在の通常ビュー／Office責務と正規遷移は画面台帳および画面遷移図、実装との差分は現行要求→画面プロト改修台帳を参照する。
...
当時の実装調査対象: `prototype/AI Office de SEO.dc.html`（単一DC）。当時の通常ビューのタブ構成は
