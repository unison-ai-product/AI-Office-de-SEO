...
| 受入トレース | `ai-office-de-seo-acceptance-trace_v3.7.md` | current | 446 REQとcanonical ACを横断追跡する。旧ACの意味も現行判断へ更新する |
...
| 機能一覧 | `docs/reference/FEATURE-LIST.md` | current | 446 REQの全数転記ではなく、現行Lifecycleと機能境界を利用者向けに要約する。全数coverageはacceptance traceを参照 |
...
- Google Sheets監査台帳は直前同期時点で分類別Markdown 20文書、248要求、267受入条件を保持する。今回の `REQ-MEASURE-13`／`AC-L1-MEASURE-13` 追加後は249要求、268受入条件への再同期が必要である。リポジトリ側の横断監査は旧詳細要求を含む446 REQ、273 canonical AC、519 traced ACで通過した。
