...
| SF-UI-16 | Plan Entitlementとデータ十分性を分離し、Site実測・業界cohort・global priorの利用比率、Upgrade／Downgradeのdata影響、通常／OfficeのMetric一致と計算精度を誤認なく操作できるか。`AOS-PRE-L3-PLAN-DATA-FIDELITY-UI-VALIDATION`のFID-UI-01〜44を比較する | Plan lock、1,000 click、記事別可否、階層weight、confidence、coverage、rollup、検索履歴、semantic metric、exact／rollup／sketch／sample、誤差、Information Mark／Popover、3秒表示。検証仕様作成済み、ブラウザ操作前のため`open` | REQ-SCREEN-16/17/21/22/24、REQ-DATA-18、Data Fidelity、Metric Contract |
