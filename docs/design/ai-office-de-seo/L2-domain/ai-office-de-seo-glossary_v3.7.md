...
| プライマリサイト／サブサイト | Standard ・ 表示中 ／ Standard（プラン名表記） | 1サイト=1プランに伴い主従表記を廃止。サイト削除は「解約して外す」 |
...
| 通常ビュー（Standard/SaaS View） | 選択の基盤（選ぶ・決める・承認・設定） | 2026-07-09 役割明確化。全操作・意思決定はこちら。Officeビューと対 |
| Agent Officeビュー（Office View） | 実行状況の確認基盤（見守る・深堀り） | 2026-07-09 役割明確化＋深堀り基盤へ格上げ（同日改訂）。「人間には面倒・難しい作業をエージェントがこなしている」様子を見せる監視面。深堀りは**Office詳細ページ**（部屋詳細の内部メニューから開く・**1ページ=1対象（1キーワード/1記事）の全詳細データ＋分析**・吹き出しワンメッセージ・スクロール絶対禁止の1画面完結・エージェントは左下配置）で行う。キーワードリサーチ=キーワード3件の各ページ、コンテンツ制作=記事2件の各ページ。通常ビューは再編せず、既存の部屋→ワークベンチ導線は維持 |
