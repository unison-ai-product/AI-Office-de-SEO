...
セカンダリが継続的に強い実績を持つ場合はプライマリとの単純入れ替えを行わない。業界別に、同一SERP、intent、co-landing、順位、表示、クリック、CV実績を再集計し、クラスタ境界、代表語、プライマリ／セカンダリ重みを再計算する。結果は業界別の辞書・算式補正候補として版管理し、Site固有較正と匿名化済み全体較正へ分けて還流する。
