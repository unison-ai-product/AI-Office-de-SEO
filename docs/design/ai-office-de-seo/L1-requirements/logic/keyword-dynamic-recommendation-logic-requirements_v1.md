...
4. AIO・広告面は地域・device・取得日時を揃える。AIOおよびAI回答面は初期提供を確約せず、提供時も月次または設定された鮮度期限到達時の観測を基本とする。週次推薦更新でAIOを毎回再取得せず、直近の有効観測をavailability・observed_at付きで使用する。広告・SERP構成は利用するProviderと原価に応じた別freshnessを持てる。
5. 全成分を0〜1へ正規化する。
...
- 週次のGSC・順位・広告／SERP構成差分、月次のAIO／AI回答面観測、または各Sourceの鮮度期限到達
- ArticleSummary更新
