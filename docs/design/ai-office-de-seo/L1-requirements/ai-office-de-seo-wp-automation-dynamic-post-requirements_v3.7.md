...
## 6. 代表記事10本未満の扱い  ［REQ-WPA-06］

生成プロファイルは代表記事10本を起点にするが、記事数がそれに満たないサイト・新規サイトも受け入れる。

- 代表記事が10本未満の場合、取得できた本数で暫定プロファイルを作り、プロファイル信頼度を「暫定」として明示する。
- 本数不足時は、Domain PositioningとUser Order、レギュレーションの比重を上げてプロファイルを補う。
...
- プラグイン操作: ブロック/要素レベルの部分更新（リンク挿入・タイトル/メタディスクリプション/H1差し替え・CTA/バナーブロック差し替え）をプラグインの操作として定義する。適用前にWPリビジョンを保存し、失敗・検証不整合時はロールバックする。適用時に取得時点との**更新競合を検知**し（本文がWP側で変更されていた場合）、安全側で停止して人へ提示する。
...
- 記録: 部分更新もSnapshot・施策台帳（`REQ-PRODUCT-19`）へ記録し、28日比較（`REQ-RWR-06`）の対象とする。
