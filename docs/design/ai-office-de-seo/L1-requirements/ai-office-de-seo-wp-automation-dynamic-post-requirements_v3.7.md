...
- プラグイン操作: ブロック/要素レベルの部分更新（リンク挿入・タイトル/メタディスクリプション/H1差し替え・CTA/バナーブロック差し替え）をプラグインの操作として定義する。適用前にWPリビジョンを保存し、WordPressのrevision取得と復元機能を第一復元経路として使用する。リビジョンが無効・削除済み・非対応の場合は、変更前データを専用の暗号化バックアップ領域へ保存して復元する。適用時に取得時点との**更新競合を検知**し（本文がWP側で変更されていた場合）、安全側で停止して人へ提示する。
