...
- ON時は利用可能な公開記事から学習対象を選定する。記事数不足を理由に導入を停止せず、利用可能な記事だけで暫定プロファイルを作り、信頼度を「暫定」として明示する。
...
- 記録: 部分更新もSnapshot・施策台帳（`REQ-PRODUCT-19`）へ記録し、反映1か月後の一次評価と3か月後の二次評価（`REQ-RWR-06`）の対象とする。
