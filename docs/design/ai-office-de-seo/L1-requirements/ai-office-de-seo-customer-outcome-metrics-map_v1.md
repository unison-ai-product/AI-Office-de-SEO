...
| 分類 | 判定 | 成果数 | 評価context |
|---|---|---|---|
| `ai_office_publication` | Publication Decision／Job、CMS Delivery、外部post ID、対象version／content hash、外部反映証拠が同じcorrelationで検証された`schema.publication.fact.v1` | AI Office公開・更新実績へ算入 | AI Office施策の評価対象。Factの`effective_at`を起点にする |
