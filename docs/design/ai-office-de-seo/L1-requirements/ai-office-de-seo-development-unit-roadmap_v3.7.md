...
ArticleSummary／Article Read Snapshot／順位・Query・記事・施策信号 → リライトRecommendation → 採用 → Cause Analysis → Edit Plan freeze → Repair Ticketまたは承認付き記事置換 → QA → Presentation Assembly／Generation Outcome → CMS Delivery → ユーザー承認。
...
- フェーズ3 生成の実証【本質Bの縦切り】: DU-10 新規記事の最小縦切り → DU-11 リライトの最小縦切り（Rewrite Recommendation → Article Read Snapshot確認 → Cause Analysis → Edit Plan → Repair／記事置換 → QA → Generation Outcome → CMS Delivery → 承認）。ここまでで本質A・Bが実データで一気通貫する。
...
- 密度の健全域: AWS上の実行単位またはworker poolあたりの**健全テナント数**を、利用率しきい値（CPU・メモリ・DB接続・ストレージ・キュー遅延・バッチ窓の消化率）で定義する。しきい値超過の接近をADM-S4で監視し、超過前にtask数、worker pool、DB容量またはtenant shardを段階拡張する。
- 段階構成: 初期からAWSへ配置し、Web/API、非同期worker、PostgreSQL、queue、object storage、監視の論理境界を保つ。小規模時は運用原価を抑えた最小構成で開始し、負荷に応じてworker分離、DB拡張、tenant shardへ進む。各段階で境界API（Gate A-2）とデータ契約を変えない。ネットワークは外向き（WP/外部API egress・許可先管理）と内向き（HTTPS終端）を分離し、集約パイプラインはアプリから直接到達不能とする（`REQ-SEC-07`）。
...
- オブジェクトストレージ: 一時本文・debug snapshot・エクスポート成果物・バックアップはAmazon S3またはS3 Contract準拠Adapterへ置き、コンテナのephemeral diskを恒久置き場にしない（DR・スケール・容量分離）。TTL物はライフサイクルで自動削除する（`REQ-SEC-11`）。
...
- 隔離保管: バックアップは本番実行環境と**別の障害ドメイン**へ暗号化保管し、同一AZ・同一実行task・同一DBだけに閉じた保管を禁止する。
...
- 段階実行: 初期からAWSのコンテナ実行環境へ不変イメージを配置し、規模に応じてtask数、worker pool、service配置を変更する。配置変更時も境界API（Gate A-2）・イメージ・データ契約を変えないことを要件とする（`REQ-DUR-06`の段階構成の実装形）。
