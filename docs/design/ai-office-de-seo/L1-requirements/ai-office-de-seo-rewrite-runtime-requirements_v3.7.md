...
WP下書き送信後は引渡し済みSnapshotとして扱い、標準WorkflowではAI Officeから再取得・上書きしない。WordPress内編集はプラグインの任意機能とし、編集差分を履歴・改善データとして取得できるが、自動再編集の起点にはしない。WordPressリビジョンと専用バックアップがともにない場合も、復元不能リスクへの権限者同意があれば実行できる。
