...
部分リライト・全文再生成とも、生成結果へ装飾を適用してWP下書きへ送り、WordPress実テーマのPreview URLでユーザーが確認し、承認後に公開記事へ更新する。AI Office内で本文・見出しを修正した場合はその差分を保護して再QAし、再生成で上書きしない。

WP下書き送信後は引渡し済みSnapshotとして扱い、標準WorkflowではAI Officeから再取得・上書きしない。WordPress内編集、AI生成版との差分表示、変更履歴取得は別機構として後続提供し、初期リリースでは自動同期しない。後続で取得した差分も自動再編集の起点にはしない。WordPressリビジョンと専用バックアップがともにない場合も、復元不能リスクへの権限者同意があれば実行できる。
...
- 効果測定: 前後CTR比較は**同順位帯での比較**とし、順位変動によるCTR変化と切り分ける。反映1か月後の一次評価では早期悪化・計測異常・観測継続・復元候補を判断し、3か月後の二次評価で中期効果、6か月後の長期評価で持続性を確定する（`REQ-RWR-06`）。成功結果はSite内較正と同意・匿名化済み全体較正の双方へ使用する。
