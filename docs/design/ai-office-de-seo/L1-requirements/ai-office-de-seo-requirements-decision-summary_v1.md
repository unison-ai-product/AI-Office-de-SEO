...
- 製品内部検索IndexはKeyword、カテゴリー／テーマ戦略、Article Summary、Recommendation、Task、顧客成果を対象とする再構築可能な派生read modelとし、SEO index診断と分離する。本文全文・Prompt・secretを索引化せず、tenant／Site Scopeを強制する。初期はlexical検索と構造化filterを必須経路とし、vectorはArticle Summary等だけの補助Capability、専用検索製品は負荷試験とADR後の選択とする。
...
- 公共外部データとして取得可能なキーワード、市場指標、関連関係、分類、時系列は、Site横断の公共キーワード資産プールへ大量・長期保持して再利用する。GSCだけで観測したクエリ、ユーザーだけが登録した語、URL、顧客別実績・対応関係は共有せず、公共資産と顧客由来の匿名集計を分離する。
- キーワードとそのclusterは需要側の「マーケット」を表し、検索量、季節性、意図、AIO・広告、競争性等で市場構造を定義する。Queryは市場で実際に発生してSiteへ表示・流入した需要の観測、記事はQuery群の順位・表示・click・CVを獲得する供給資産であり、Queryと記事の対応を「シェア」として評価する。市場規模・市場価値と、自Siteの記事が獲得したシェアを同じ指標へ混ぜない。
...
- 新規Siteは、本システムで生成され、人が完成記事を承認し、`effect_kind=new_publish / attribution=ai_office_publication / reconciliation=confirmed`のPublication Factが成立した新規記事15件まで個別承認を必須とする。予約、CMS API受付、下書き、外部変更、帰属確認中、既存記事、リライトは算入しない。
...
- Agent Officeは実行中Task、担当Agent、工程、待機、完了、失敗を見守りながら、成果のSite／Cluster／記事詳細、根拠、条件、推薦方針、設定、Taskを玄人向けに分析・操作する面とする。定型指示は選択式ポップアップと決定論Service、自由文は必要時だけLLMを使う。変更は型付きProposal、影響・Credit・認可確認を経て共通Commandへ渡す。通常ビューは同じ成果の要約と簡単操作を担う。
- Officeの役割別Agentは、既存Executor・決定論Service・Workflowの実状態を業務用語へ翻訳し、担当領域の詳細分析・設定・Task操作へ接続するユーザー接点とする。各Agentを独立Runtimeに固定せず、現在Task、工程、待機理由、完了成果、根拠、利用可能な調整操作を説明する。
...
- Article読取りの経路選択は内部制御とする。顧客が設定するのはSite URL、CMS接続、認証、Plugin導入等、選択機能の利用に必要な項目だけとし、Crawler、REST、primary／standby、優先順位、取得頻度、fallback、負荷制御を設定させない。顧客画面は業務の利用可否、影響、必要な操作だけを示し、詳細診断は開発管理面とFAQチャットの切り分け根拠へ使用する。
- 公開記事差分は機械処理を基本とし、CTA・CV経路、SEO評価対象、取得・公開error、軽微変更の順に分類する。必要な評価起点だけを更新し、意味要約等の規則処理で作れない派生情報に限ってLLMを使用する。
