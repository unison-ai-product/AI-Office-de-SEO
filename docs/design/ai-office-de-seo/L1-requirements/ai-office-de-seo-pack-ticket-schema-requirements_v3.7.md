...
| `source.keyword.map.v1` | キーワードマップ | `clusters[{cluster_id, representative, state, keywords[{normalized, role, market_state}], serp_similarity, intent_mix, assigned_articles[], competitor_cohort[], traffic_potential_range, site_attainability{components,confidence}, attributes{target_fit,industry_fit,ymyl_adjacent,market{aio_pressure,paid_pressure,domain_credibility_fit,availability},strategy{site_necessity,traffic_opportunity,conversion_opportunity,content_feasibility,profile,dynamic_priority,components,confidence},value_score},provenance[],version}]`（決定論付与、`REQ-KGA-13/17/23`・`REQ-KRL-02〜08`） |
...
- production_quality（可読性）: Flesch Reading Ease 目標60〜70（一般読者。専門・技術読者は下振れを許容）。受動態≤10%、20語超の文・300語超の見出しセクション・長段落を警告。※可読性は直接の順位要因ではなく読者層で調整。
