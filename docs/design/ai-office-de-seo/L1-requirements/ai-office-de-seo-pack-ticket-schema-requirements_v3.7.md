...
| `source.keyword.map.v1` | キーワードマップ | `keywords[{normalized, layer, target_group, class, attributes{intent, commercial, target_fit, industry_fit, ymyl_adjacent, value_score, aio_suppressed(bool|unknown)}}]`（attributesは決定論付与、`REQ-KGA-13`/`REQ-KGA-17`） |
