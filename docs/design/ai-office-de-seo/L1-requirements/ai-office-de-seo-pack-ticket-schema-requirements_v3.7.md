...
| `source.keyword.map.v1` | キーワードマップ | `keywords[{normalized, layer, target_group, class, attributes{intent, commercial, target_fit, industry_fit, ymyl_adjacent, market{aio_pressure, paid_pressure, domain_credibility_fit, availability}, strategy{site_necessity, traffic_opportunity, conversion_opportunity, profile, dynamic_priority, components, confidence}, value_score}}]`（attributesは決定論付与、`REQ-KGA-13/17/23`） |
