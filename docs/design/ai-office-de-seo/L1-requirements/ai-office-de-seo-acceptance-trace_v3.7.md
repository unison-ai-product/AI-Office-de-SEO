...
- [ ] AC-SUMM-01: ArticleSummaryが契約（見出しツリー・要旨上限・意味ユニット・tier/タグ・hash・更新日）で差分更新され、本文全文を含まず、リンク文脈・独自性予備判定・ブリーフがサマリー参照で行われ本文取得が省略され（効果をADM-S4観測）、埋め込みはサマリー由来・用途列挙・決定論一次判定の補助に限定され、モデルversion固定・変更分のみ再計算・テナントスコープである。 ｜ 検証: REQ-PRODUCT-20, REQ-PRODUCT-04, REQ-KGA-08
