...
顧客成果は、顧客Siteに対する代行結果として顧客へ見せる指標であり、運営側KPIとは別の表示・評価契約を持つ。最低限の候補は検索流入、表示回数、獲得keyword、keyword／cluster順位、公開・更新数、CV、内部link・cluster充足、3位以内保護対象である。市場全体の検索量、AIO・広告出現、季節性、計測欠損を併記し、単純な前後差を本サービスの因果または保証成果として表示しない。正式な算式、評価周期、画面表示は別要求で定義する。
