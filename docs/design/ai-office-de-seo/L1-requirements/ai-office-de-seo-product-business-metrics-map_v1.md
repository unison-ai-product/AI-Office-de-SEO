...
顧客成果は、顧客Siteに対する代行結果として顧客へ見せる指標であり、運営側KPIとは別の表示・評価契約を持つ。検索流入、表示回数、獲得keyword、keyword／cluster順位、公開・更新数、CV、内部link・cluster充足を対象とし、市場全体の検索量、AIO・広告出現、季節性、外部変更、計測欠損を併記する。正式な算式、評価周期、S1／S2／S5への画面割当て、Office監視境界は `REQ-MEASURE-14` と `ai-office-de-seo-customer-outcome-metrics-map_v1.md` を正本とする。3位以内から導出するprotectは成果指標ではなく運用flagとして分離する。
