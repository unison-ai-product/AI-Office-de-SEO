...
- [ ] AC-TECH-01: L2構成図で責務、所有データ、同期・非同期境界が追跡できる。
- [ ] AC-TECH-02: 通常DBに本文全文、生HTML、長い外部レスポンスが保存されない。
- [ ] AC-TECH-03: 本文一時処理の成功・失敗双方で期限内削除が検証される。
- [ ] AC-TECH-04: 未変更記事・キーワードが再解析対象から除外される。
- [ ] AC-TECH-05: 長時間処理が同期APIを占有せず状態を追跡できる。
- [ ] AC-TECH-06: ジョブ再起動後も状態と固定versionから処理を継続できる。
- [ ] AC-TECH-07: 再試行で二重課金・二重公開・二重通知が発生しない。
- [ ] AC-TECH-08: 結果整合処理の失敗を検出し再処理・照合できる。
- [ ] AC-TECH-09: DB、キャッシュ、キュー、検索、ログでtenant/site境界が検証される。
- [ ] AC-TECH-10: Provider停止時に設定変更で停止・切替・fallbackできる。
- [ ] AC-TECH-11: API・イベントのversion互換性が契約テストで検証される。
- [ ] AC-TECH-12: 実行中ジョブが設定変更の影響を受けず再現できる。
- [ ] AC-TECH-13: キャッシュ、キュー、一時オブジェクトに容量・期限・異常監視がある。
- [ ] AC-TECH-14: 非本番から本番データ・秘密情報・課金経路へ到達できない。
- [ ] AC-TECH-15: アプリ・DB変更を段階適用し、失敗時に復旧できる。
