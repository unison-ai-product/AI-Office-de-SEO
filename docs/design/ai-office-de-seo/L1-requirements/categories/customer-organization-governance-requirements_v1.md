...
- `契約責任者`: 契約者を代表する利用者。必ず1名置き、契約通知と最終連絡先を担う。
- `契約管理者`: 契約、請求、プラン、追加クレジット等を管理できる利用者。複数名設定できる。
...
- `基本権限`: サイト責任者、サイト管理者、サイト利用者の3種類から選ぶSite運用権限。
- `Business Role Tag`: SEO運用で担当する役割を表す業務タグ。
- `Site Assignment`: MembershipがアクセスできるSiteを任意に絞り込む指定。

基本構造は `契約者 → 顧客組織 → Membership（基本権限 + 業務タグ + 任意のSite Assignment）` とする。契約責任者・契約管理者の契約権限と、Siteを操作する基本権限を分離する。業務タグごとにSite範囲を分けず、MembershipのSite Assignmentをすべての業務タグへ共通適用する。

## 3. 要求

### REQ-ORG-01 組織階層

すべての契約で顧客組織を必須とし、その配下にユーザーが名称と親子関係を自由設定できるOrganization Unitを構成し、Siteを任意の組織ノードへ所属させられなければならない。事業部、部門、チーム、ブランドは初期候補であり、固定階級にしない。個人契約は本人を契約責任者とする初期組織から開始できる。組織階層の深さ、子ノード数、名称長には設定可能な上限を持ち、循環参照を禁止する。
...
顧客側権限は「契約権限」「Siteの基本権限」「業務タグ」に分離する。

契約権限は次の2種類とする。

- `契約責任者`: 必ず1名。契約管理者の追加・削除、契約終了、契約責任者の交代を行える。
- `契約管理者`: 複数名可。請求、プラン、追加クレジット、契約設定を管理できるが、契約終了と契約責任者の交代は行えない。

Siteの基本権限は次の3種類とする。

- `サイト責任者`: Siteの予算、接続、利用者、基本権限、業務タグ、自動運用を管理する。
- `サイト管理者`: Siteの日常設定とサイト利用者への業務タグを管理するが、サイト責任者の追加・削除は行えない。
- `サイト利用者`: アクセス可能なSiteの情報を閲覧できる。業務状態を変更する能力は業務タグで追加する。

契約責任者または契約管理者であることだけを理由にSiteへアクセスできない。Siteを操作するにはMembership上の基本権限を別に持つ。閲覧だけの利用者は業務タグを持たないサイト利用者として表現する。SEO運用上の変更能力は、実際の担当業務に対応する次の4タグだけで付与する。
...
外部接続、Membership、基本権限、業務タグ、Site予算の管理はSEO業務タグにせず、サイト責任者／サイト管理者の権限として扱う。請求、プラン、追加クレジット、契約は契約責任者／契約管理者が扱う。サイト責任者はサイト管理者／サイト利用者の基本権限と業務タグを付与・取消でき、サイト管理者はサイト利用者の業務タグを付与・取消できる。自動運用の有効化・停止は、対象業務のタグに加えてサイト責任者またはサイト管理者の権限を要求する。
...
SEO業務状態を変更する操作は、REQ-ORG-03の4つの業務タグで制御する。サイト利用者はタグなしでもアクセス可能なSiteのDashboard、Recommendation、記事、Task History、完成済み分析結果を閲覧できるが、設定や業務状態を変更できない。記事生成、戦略変更、分析設定等のSEO業務は、サイト責任者またはサイト管理者であることだけを理由に許可せず、対応する業務タグも確認する。顧客が閲覧するTask Historyと開発側の内部監査ログを分離し、顧客権限から内部監査ログ、trace、stack、秘密情報、他tenant・管理操作詳細へ到達できない。
...
記事、キーワード、分析、成果等の顧客業務データは、MembershipのSite Assignmentに従って可視化する。Site Assignmentがない場合は全Site、指定がある場合は指定Siteの情報だけを表示する。請求・契約情報はSite Assignmentではなく契約責任者／契約管理者の契約権限で制御する。内部監査情報は顧客面へ表示しない。

### REQ-ORG-09 契約主体と組織

契約者は法人または個人を契約主体として識別し、請求主体と顧客組織を対応付ける。契約責任者は必ず1名、契約管理者は複数名設定できる。法人・個人で記事、推薦、組織、権限の機能経路を分岐させず、契約・請求に必要な属性だけを区別する。代理店による顧客横断管理、OEM、ホワイトラベルはスコープに含めない。
...
契約責任者・契約管理者、所属、基本権限、業務タグ、承認、代理操作、予算配賦の変更について、実行者、承認者、対象、変更前後、理由を監査記録へ残す。契約責任者とサイト責任者は担当範囲の権限棚卸しを実施でき、未使用・過剰な業務タグの候補を確認できる。
...
- [ ] AC-L1-ORG-02: 契約権限とSiteの基本権限が分離され、同一ユーザーが所属先ごとにサイト責任者／サイト管理者／サイト利用者と業務タグを持てる。
...
- [ ] AC-L1-ORG-05: 目標管理、キーワード・サイト戦略、記事制作、サイト分析の4タグで変更操作を制御し、契約管理とSite管理を日本語の別権限で制御できる。
- [ ] AC-L1-ORG-06: 多段階承認、差し戻し、期限、代理承認が監査される。
- [ ] AC-L1-ORG-07: 部門・Site予算の超過が実行前に停止または承認待ちになる。
- [ ] AC-L1-ORG-08: 個人契約が本人を契約責任者とする初期組織から法人契約と同じ機能を利用できる。
- [ ] AC-L1-ORG-09: 退職・契約終了時にアクセスが失効し、成果物と履歴は失われない。
- [ ] AC-L1-ORG-10: 組織・Site移管前に権限、データ、請求への影響が表示される。
- [ ] AC-L1-ORG-11: 契約責任者とサイト責任者が担当範囲の権限棚卸しを実行し、是正履歴を確認できる。
