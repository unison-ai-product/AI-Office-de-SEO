...
個人・小規模利用向けの初期商品仮説として、最低品質段階と最小クレジットを含む月額19,800円のEntry Planを置く。一般消費者も申込み可能な公開価格は総額表示を正本とし、現時点では月額19,800円（税込）を企画基準値とする。法人向け見積・請求で税額を分離表示する場合も、顧客が支払総額を確認できるようにする。法人・個人で別システムを作らず、利用量、品質、組織機能、バックアップ、サポート範囲を商品差とする。19,800円は販売確定価格ではなく、β原価・需要検証前の企画基準値としてversion管理する。
