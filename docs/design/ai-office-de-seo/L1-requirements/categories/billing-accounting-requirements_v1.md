...
| カスタマイズ | 対象外 | 対象外 | 要相談・別見積 | 要相談・個別設計／別見積 |
...
カスタマイズはPremiumおよびEnterpriseの相談導線として表示する。Premiumは既存Feature Object／拡張アプリの選定、設定、組合せ、軽微な個別調整を中心に有償見積できる。Enterpriseは業務整理、独自Feature Object、外部連携、Workflow／Pack、画面／Office拡張、移行、検証、保守まで個別設計できる。月額利用料に無制限な開発工数を含めず、相談、要件整理、構築、第三者費用、保守・version追従の範囲と料金をStatement of WorkおよびPrice Catalog商品として分離する。

要相談表示は実現保証ではない。技術適合、セキュリティ、保守性、共通化可能性、Provider規約、納期、原価を確認し、Coreの安全境界を破る変更は受注しない。顧客専用実装であっても可能な範囲でFeature Object／App Packageとして隔離し、Core forkを標準提供形態にしない。
...
無料Trialは一般公開・常設せず、初期検証全体で累計10社へ限定する。終了したTrial枠を別会社へ再利用せず、11社目を発行できない。Platform Adminが対象Customer Organizationごとに1カ月以上3カ月以内の期間、Standard相当のTrial Plan version、固定credit、Site・ユーザー数、機能、データ容量を指定して招待・発行する。専用バックアップ等の上位機能は既定で含めず、検証目的がある場合だけ期限付き個別Entitlementで付与する。公開セルフサインアップ、紹介コードの無制限拡散、同一主体の反復取得を許可しない。

Trialでも最初の新規15記事は完成記事の承認を必須とし、通常契約と同じ承認実績、同意、設定を満たした場合だけ自動投稿を解放する。Trial区分を承認・品質・公開条件の迂回根拠にしない。
...
### REQ-BILLING-17 Plan別Data Fidelity Entitlement

Plan Configurationは、Keyword／cluster／競合等の観測coverage、外部データfreshness、詳細粒度、詳細保持期間、再計算頻度、Site固有feature depth、外部取得予算、検索可能履歴を独立Dimensionとして持つ。上位PlanはこれらのCapacityを増やしてglobal priorへのfallbackを減らせるが、顧客実測値への加点、標本条件の緩和、精度保証を商品権利にしない。具体件数はβ原価実測後にversion付き設定へ登録する。

Upgrade Previewは追加される観測対象、詳細期間、比較軸、再計算頻度、予測可能対象を示す。Downgrade Previewは失われる詳細、rollup・削除予定、export期限を示し、過去Observation Factを変更しない。内部検索、API、Jobは同じEntitlement Snapshotを参照し、検索IndexをPlan制限の迂回経路にしない。詳細は`ai-office-de-seo-plan-data-fidelity-connection-map_v1.md`を正本とする。
...
- [ ] AC-L1-BILLING-17: Plan別Data Fidelityを観測coverage・freshness・粒度・保持・再計算等のDimensionで付与し、Upgrade／Downgradeのデータ影響をPreviewでき、Planを顧客実測への加点・標本条件緩和・精度保証として扱わない。
