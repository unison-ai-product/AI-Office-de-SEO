...
個人・小規模利用向けの初期商品仮説として、最低品質段階と最小クレジットを含む月額39,800円（税別）のEntry Planを置く。一般消費者も申込み可能な公開価格では、税抜39,800円を主表示する場合も、同じ価格表示領域へ税込43,780円を明瞭に併記する。税抜価格だけを過度に強調して支払総額を誤認させない。法人向け見積・請求で税額を分離表示する場合も、顧客が支払総額を確認できるようにする。法人・個人で別システムを作らず、利用量、品質、組織機能、バックアップ、サポート範囲を商品差とする。39,800円は販売確定価格ではなく、β原価・需要検証前の企画基準値としてversion管理する。
