...
| Prime | 198,000円 | 217,800円 | セルフ申込・年契約のみ |
...
EntryとStandardは1か月単位で契約でき、希望者は12か月契約を選択できる。Primeは12か月契約だけをセルフ申込できる。Enterpriseは問い合わせ・個別見積による12か月契約だけを提供する。

12か月契約はシステム利用料を10%割引し、追加credit、初期設定、個別導入、コンサルティング、オプション、従量利用は割引対象外とする。年契約の途中解約、更新、Plan変更、未使用creditの扱いは契約前に表示し、割引と別キャンペーンの併用可否をPrice Catalogへ持つ。EntryとStandardはセルフ導入を基本とし、有償の導入支援を別商品として追加できる。Primeの有償導入支援は申込時に追加でき、Enterpriseは問い合わせ時に対象Site、組織規模、移行・接続、初期設定、運用体制、必要支援を確認する。
...
追加容量商品を購入できるのはPrimeおよびEnterpriseとする。EntryとStandardが容量上限へ接近した場合はPrimeを推奨し、追加容量だけを販売しない。この販売可否、対象Dimension、追加量、価格、期間はPlan ConfigurationとPrice Catalogから変更可能にする。
...
- [ ] AC-L1-BILLING-02: Entry・Standardの月契約、Primeのセルフ年契約、Enterpriseの問い合わせ年契約および年契システム利用料10%割引を再現でき、内部契約と外部Subscriptionの状態差を検出して未検証Webhookで利用権限が直接変更されない。
