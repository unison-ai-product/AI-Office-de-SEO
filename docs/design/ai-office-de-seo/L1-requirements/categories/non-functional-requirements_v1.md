...
同期経路は入力検証、表示変更、軽い参照、非同期Command受付へ限定し、外部Provider、AI、CMS反映確認、全件分析を同期request内で完了させない。受付応答はTask参照、初期状態、取消可否を返し、画面描画とTask実行を別の性能予算で計測する。バックグラウンド処理中もNavigation、既存成果閲覧、別Task受付を利用可能に保つ。
...
顧客が到達可能な全ページは、バックグラウンド計算、外部取得、Office assetの完了を待たずにroute shell、Navigation、ページ名、利用可能な操作を表示する。Dashboard、Keyword、Report、月次計画、Recommendation、記事、Task、CMS状態、成果、設定等のデータ画面は、標準利用条件のP95で遷移開始から3秒以内に、判断可能な既存data、前回確定snapshot、部分結果、または理由・更新時刻・次回更新を持つ正直な状態のいずれかを表示する。3秒以内に最新再計算を完了させる要求ではなく、空白、全面spinner、処理完了待ちでページ自体を隠すことを禁止する。

標準利用条件はdesktop、対応browser、基準network、基準Site規模、warm／cold cacheを分けてversion管理する。3秒SLOは画面種別、dataset規模、cache条件別に計測し、範囲外の大規模Siteもページを表示したうえで段階読込、server-side paging、部分開放、自動構築状態へ縮退する。未権限・未契約・未接続ページもrouteを消さず、理由と解消操作を3秒以内に表示する。
...
処理種別ごとに受付時間、queue待機、実行時間、外部待機、ユーザー待機、成果提供までを分解して測定する。UIに出す見込みは同種処理の観測分布と現在のqueue／Capacityから範囲で算出し、算出不能時は時刻を捏造しない。Task ShelfとOffice Projectionのevent反映遅延も計測し、実処理状態との乖離を監視する。
...
- [ ] AC-L1-NFR-02: Core Web Vitalsと主要画面・ジョブ受付のP50/P75/P95を継続計測でき、顧客向け全page routeを先に表示したうえで、データ画面が標準利用条件P95 3秒以内に判断可能なdata、部分結果または理由付き状態へ到達する。最新再計算待ち、Plan、権限、接続不足を理由にpageを隠さない。
