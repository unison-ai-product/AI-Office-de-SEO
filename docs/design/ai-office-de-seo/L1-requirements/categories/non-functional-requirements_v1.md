...
通常利用条件における主要画面の初期操作可能時間、画面遷移、主要操作応答をP50/P95で計測する。初期目標は、キャッシュ済み主要画面の操作可能時間P95 2.5秒以内、画面内操作応答P95 300ms以内、ジョブ受付P95 1秒以内とし、実測で改訂する。
...
重要データごとにRPO/RTO、バックアップ頻度、保持、復元手順を定義する。初期値は課金・権限・設定・施策台帳をRPO 24時間以内、RTO 8時間以内の上限候補とし、商用開始前の復元演習結果から短縮要否を決定する。
...
- [ ] AC-NFR-02: 主要画面・操作・ジョブ受付のP50/P95を継続計測できる。
...
- [ ] AC-NFR-07: バックアップから目標RPO/RTO内に復元できる。
- [ ] AC-NFR-08: 外部障害時も既存データ閲覧と状態確認ができる。
