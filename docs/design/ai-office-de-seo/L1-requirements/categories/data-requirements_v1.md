...
Siteは「業界／業種」の2階層分類を複数持てる。業界は上位分類、業種はその配下の下位分類とし、3階層以上へ深くしない。複数の業界／業種を設定する場合、ユーザーは各分類を横断してSiteが扱う共通テーマ、対象顧客、課題、商品軸等の横断軸を明記する。標準Catalogから選択できるほか、ユーザーがSiteに必要な業界・業種を追加できる。ユーザー追加値は作成者、Site、親分類、正規化名、作成日時、状態を持ち、当該Site内の分類として保持して標準Catalogを直接改変しない。匿名全体較正への昇格は通常のk匿名・Catalog改版手順を通す。
...
- [ ] AC-DATA-11: Siteへ業界／業種の2階層を複数設定でき、複数設定時は横断軸を保持し、ユーザー追加分類が標準Catalogを直接変更しない。
