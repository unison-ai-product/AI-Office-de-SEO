...
記事ごとに獲得キーワード、検索順位、表示、クリック、CTR、CV、公開・更新、推薦、施策、品質結果の時系列を記事遍歴として参照できる。Tracker受信eventはSite、event ID、event type、発生時刻、現在URL、直前遷移元URL、明示CTA／button ID、該当CV Goal、同意状態、tracker／定義versionだけを許可し、個別user ID、session ID、IPの永続値、フォーム内容、複数ページ経路を持たない。受信eventは重複排除・bot除外・schema検証後、page viewは日×URL、遷移は日×遷移元×遷移先、CTAは日×URL×CTA、CVは日×到達URL×Goalおよび日×直前遷移元×到達URL×Goalへ集約する。生eventは処理TTLまたは隔離期限後に削除し、恒久正本は集計値と欠損・除外countだけとする。特定ボタンからの遷移は確実に識別できる場合だけbutton／CTA識別子を付与し、サンクスページ等の明示された到達ページをCVとして判定する。
...
- [ ] AC-L1-DATA-07: 施策、獲得キーワード、順位、確実なページ遷移・CV、公開・更新の遍歴を追跡でき、Trackerの恒久保存が日別集計と欠損・除外countに限定され、user／session／複数ページ経路の保存tableが存在しない。
