...
記事ごとに獲得キーワード、検索順位、表示、クリック、CTR、CV、公開・更新、推薦、施策、品質結果の時系列を記事遍歴として参照できる。高頻度の生データを無期限保持せず、期間別集計と重要イベントへロールアップする。
...
- [ ] AC-DATA-07: 施策、獲得キーワード、順位、CV、公開・更新の遍歴が元記録を改変せず追跡できる。
