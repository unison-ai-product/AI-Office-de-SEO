...
レコメンドの目的は、流入増加、CV増加、サイト情報充足、既存記事の保護・改善、費用対効果等からユーザーがSite単位で単純選択できなければならない。ロジックは選択された目的を達成するため、新規記事、リライト、統合、内部リンク、CTA、保護、監視の必要配分を算出する。ユーザーへ複雑な重み入力を要求せず、製品も単一の事業目標を全Siteへ固定しない。
...
自動投稿は、当該Siteで公開成功15記事へ人間承認が記録され、権限者が版付き同意書へチェックし、対象Site・記事種別・変更種別、予算、品質ゲート、公開時間、変更量、cooldown、外部接続状態を設定した場合だけ解放する。重大品質不合格、YMYL追加条件、好調記事保護、予算超過、stale根拠、Kill Switchは常に自動公開を停止する。

## 受入条件

- [ ] AC-LOGIC-01: Siteごとに目的を単純選択し、その達成に必要な施策種別・記事配分が計算される。
