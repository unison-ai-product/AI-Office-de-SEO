...
新規記事またはリライトのRecommendation Item採用を起点に、Ticket生成、調査、構成、生成・編集、品質検査、CMS Delivery、承認判定、予約・公開、効果計測をversion付きイベントで接続する。初期WordPress AdapterではCMS DeliveryからWordPress下書きへ接続する。各段階は冪等で、停止・再開・取消・再試行可能とする。
...
新規記事の自動投稿は、当該Siteで本システム作成の新規記事15件へ完成記事の人間承認証拠と、`effect_kind=new_publish / attribution=ai_office_publication / reconciliation=confirmed`のPublication Factが記録され、権限者が版付き同意書へチェックし、対象Site・記事種別、予算、品質ゲート、公開時間、変更量、cooldown、外部接続状態を設定した場合だけ解放する。予約、CMS API受付、下書き、外部変更、帰属確認中を15件へ算入しない。既存記事とリライトを15件へ算入しない。

リライトと全文再生成はCMS下書きまで自動化できるが、公開記事への更新はユーザー承認を必須とする。初期WordPress AdapterではWordPress下書きを使用する。重大品質不合格、YMYL追加条件、虚偽・欺瞞、スパム、構造破壊等のhard gate該当成果は通常承認だけでは更新できない。同一権限者が対象、警告、未解消項目、責任境界を二段階で確認し、版付き同意書へ同意した場合だけ手動公開できる。異なる2名による承認は必須としない。予算超過、stale根拠、Kill Switchは引き続き公開を停止する。
...
画像の生成、Pattern設定、WordPress Media登録、featured media設定は制作ツールとして提供する。生成画像の採否と公開判断はユーザーに帰属し、人物、商品、実績等を含むことだけを理由に本システム側で個別承認を追加しない。

自動投稿時の画像gateは、ファイル破損、CMS非対応形式・容量、アップロード失敗等の技術的不成立と、ユーザーがPatternへ明示した禁止要素、欠落した必須素材等の設定不一致に限定する。構図・トーン・バリエーション等の主観的差異はadvisoryとし、承認済みPatternの自動投稿を過剰に停止しない。

CMS Adapter、署名Webhookまたは許可された観測経路から外部の公開・更新通知を受信した場合は、CMSの更新日時を記事の最新変更日時として遍歴へ追加する。これは記事変更履歴および既存評価の交絡候補であり、過去Snapshot、当時の施策・評価記録、AI Office帰属を上書きしない。変更内容を取得できない場合は、更新日時だけを根拠に変更量、SEO影響または`ai_office_publication`を断定しない。初期WordPress Adapterでは署名WebhookとREST再取得を優先する。
...
AIOおよびAI回答面の観測は月次を基本とし、短期間の表示差へ追従して反復取得しない。急変検知は即時再取得トリガにせず、追加観測はユーザー明示実行またはProvider仕様・障害の検証等へ限定する。

### REQ-LOGIC-06 公開ページ段階評価

`ai_office_publication`として外部反映を検証したPublication Factの`effective_at`を起点に、1か月後に一次評価、3か月後に二次評価、6か月後に長期評価を実行する。予約、CMS API受付、下書き、外部変更、帰属確認中だけでは評価を開始しない。評価時点では「新規記事」という制作時分類を使わず、公開ページの獲得キーワード、順位、クリック、CV、ページ状態を評価する。
...
### REQ-LOGIC-13 公開記事の変更分類と評価起点

公開記事の変更検知と分類は、HTTP状態、CMS更新日時、content hash、DOM構造、title／meta、見出し、本文section、CTA、内部link、装飾属性の機械比較を正本とする。LLMは機械比較だけでは判定できない検索意図・主張・意味変化の補助分類に限定し、同一変更を判定するたびにAgentまたはLLMを起動しない。

処理優先度は `CTA・CV経路 > SEO評価対象 > error・取得状態 > 軽微変更` とする。CTA、遷移先、CV導線の変更はCV評価の起点を更新する。title、meta、主要見出し、検索意図へ影響する本文変更はSEO評価の起点を更新する。404、redirect、非公開、取得不能等は成果悪化へ混入させず接続・公開状態の診断へ送る。誤字、余白、色、装飾、意味を変えない軽微変更は履歴だけを更新し、SEO／CVの評価周期をリセットしない。内部linkの追加・変更はlink graphを差分更新し、削除は既定より高い注意状態とする。

公開表示はSEO評価、CMS保存値は編集・更新、Plugin Snapshotは変更通知、履歴Snapshotは復元参照に使用する。すべてを常時取得せず、利用可能な変更信号から必要な用途の正本だけを差分取得する。source間の差は障害ではなく用途差として保持し、取得時刻とsourceを表示可能にする。404、下書き化、ゴミ箱、redirect、URL変更、一時取得失敗を区別し、単発失敗で削除確定しない。

意味要約、検索意図、主張・根拠等、規則処理だけでは生成できない派生情報に限りLLM処理を許可する。同一content hashとrule versionでは再実行せず、機械的な部分差分だけで成立する集計、link graph、状態更新をLLMへ送らない。

### REQ-LOGIC-14 CV Goal・CTA接続
...
- [ ] AC-L1-LOGIC-04: 人間承認証拠付き新規`ai_office_publication` Fact 15件、版付き同意、事前許可のいずれかが欠ける場合、または停止条件下で自動投稿が実行されず、予約・下書き・API受付・外部変更・帰属確認中・リライトを15件へ算入しない。
