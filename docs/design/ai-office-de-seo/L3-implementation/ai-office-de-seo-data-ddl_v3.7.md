...
対象: wp_capability_snapshots（snapshotKey / schemaVersion）、dynamic_post_schemas、publication_jobs（dynamicPostSchemaKey・slot assignment metadata・content hash・validation result・WP draft URL・job result・correlation_id）、publication_decisions / publication_confirmations / approved_new_article_counters（`schema.publication.decision.v1`。15件は公開成功eventから導出し、任意更新しない）、scheduled_actions / automation_policies / approval_requests / content_calendar_slots、recommendation_feedback / saved_views / user_exploration_sessions。`recommendation_items`と`recommendation_intakes`の所有ContextはSearch Performanceとし、Publishingは参照だけを持つ。
