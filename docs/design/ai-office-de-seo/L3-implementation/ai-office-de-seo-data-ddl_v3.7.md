...
- `metric_snapshots`はmetric version、tenant／Site Scope、subject、grain、Dimension、window、valueまたはrange、availability、confidence、coverage、watermark、Calibration、calculation mode、algorithm version、母集団／sample size、precision／error bound／confidence interval、bias note、計算時刻、Projection versionを持つ再構築可能なread modelとする。通常／Office別の値列を作らず、画面差はPresentation Projectionで表現する。Exact値にSampling metadataを偽装せず、Sampling値に母集団・frame・method・weight・seed・誤差を欠いたrowを許可しない。
...
- `publication_facts`は`effective_at`に加え、`effective_time_source`、`effective_time_precision(exact/estimated)`、`source_observed_at`、`clock_skew_checked_at`、`effective_time_rule_version`を必須とする。予約、Command、API受付、Webhook受信、検証終了時刻からの代入を禁止し、許容外の未来値・不合理な過去値・Source不一致はconfirmed制約を満たさない。`unique(tenant_id,site_id,external_post_ref,effect_kind,resulting_content_hash,effective_at,verification_evidence_ref)`で同じ外部事実を冪等化する。
- `site_activations`は`unique(tenant_id,site_id)`とfirst qualifying Fact参照、`approved_new_article_fact_memberships`は`unique(tenant_id,site_id,publication_fact_id)`、`evaluation_lanes`は`unique(tenant_id,site_id,publication_fact_id,intervention_version,lane_type)`を持つ。`product_loop_completions`はFact、baseline、SEO Lane、1／3／6か月checkpointと同一transactionのoutbox参照を一意に保持する。派生Consumerの失敗・再送はFactを更新せず、各一意制約から再開する。
...
- `output_vault_provisions`は非公開staging object ref、tenant／Site／job、Presentation Snapshot、content hash、size、QA seal、read-after-write verification、state、created／verified／cleanup due／cleaned時刻だけを保持する。Provision objectはOutcome確定前にユーザー取得用署名URLを発行しない。`generation_outcomes`、commit Ledger row、outbox rowsは同一transactionで作成し、`generation_outcomes.credit_commit_ref`を非NULLかつ一意参照、`unique(tenant_id,site_id,intake_ref,content_hash)`とする。失敗transactionのProvisionはorphan cleanup queueへ送る。
- `output_vault_availability`はOutcomeとは別Projectionとしてavailable／expiring／expired／deletion_pending／deleted／unavailable_incident、期限、削除証跡、pending Delivery参照を保持する。期限削除でOutcomeまたはcommit Ledgerをupdate／deleteせず、CMS Deliveryを生成失敗へ戻さない。本文payloadはobject storageに限り、tableへ複製しない。
