...
- `recommendation_items`は`recommendation_id + version`を安定keyとし、正規type、subtype、target_ref、objective_ref、keyword_cluster_ref、search_intent、article_purpose、reason_evidence_refs、cta_policy_ref、internal_link_plan_ref、quality_tier、budget_estimate、protection_policy、availability、dependencies、score_components、status、expires_at、supersedes_ref、origin（monthly_plan/weekly_refresh/triggered_event）を保持する。typeはAction Routing MapのCatalogへ制約し、判定alias、Workflow名、画面名を保存しない。採用時は`recommendation_intakes`へ`schema.intake.recommendation.v1`準拠のfreeze済み入力、route、contract hash、correlation_idを保存し、各workflow／patch／policy／user taskから参照する。手動指定は`manual_intakes`へ`schema.intake.manual.v1`準拠で保存し、Recommendation採用を偽装しない。両Intakeは画面表示値から再構築せず、Preflight結果をIntake rowへ上書きしない。ユーザー指定Taskとの衝突では自動予定側をneeds_reviewへ戻し、手動Taskを暗黙取消ししない。
