...
| Metric要約 → 通常ビュー内drilldown → Office詳細 | 同じMetric Definition、Scope、grain、window、filter、attribution、versionへの閲覧権限 | 閲覧は共通。分析条件変更はサイト分析、戦略変更はキーワード・サイト戦略 | 同じMetric SnapshotをS1／S2／S5から通常詳細へ掘り下げ、さらにOfficeでCalibration、watermark、根拠を表示。戻る時は対象・期間・filterを復元 | Plan lock、data insufficient、partial、staleを分離する。Officeで再計算せず、詳細遅延中も3秒以内に状態と利用可能操作を表示 | REQ-SCREEN-16・18・21 / REQ-TECH-21 / REQ-DATA-17 |
...
| S7 Plan変更影響 → Upgrade／Downgrade確定 | 現行Entitlement Snapshot、変更候補Plan Configuration、影響計算versionが成立 | 契約者。支払・契約変更はstep-upを追加適用 | 新Entitlementのeffective timeを確定し、将来Query／取得Capacityへ適用。通常／Officeへ同じAvailabilityを反映 | 順位・精度保証を表示しない。Downgradeはrollup、詳細履歴失効、export期限を確認させ、過去Observation Fact・確定評価・Metric Definitionを変更しない | REQ-BILLING-17 / REQ-SCREEN-17・22 / REQ-ACCESS-08 |
