...
> **2026-08-03 現行要求による上書き**: 通常ビューは分析、判断、設定、承認、変更、成果drill downを行う業務面、Agent Officeはエージェントの実行状況を見守る監視面とする。OfficeはTask、工程、待機、完了、失敗、成果要約と通常ビューへのContext付きlinkを持ち、独自の成果分析・Keyword／記事drill down・詳細設定を持たない。以下の詳細操作面・成果Explorerに関する記述はプロトタイプ実装履歴であり、現行要求として使用しない。

**現行役割（2026-08-03）**: Officeビューは実Task・Agent・工程eventの監視面である。既存の部屋、Agent、ライブ表示は再利用し、成果詳細PanelはSite＝S1、Cluster＝S2、記事＝S5への要約linkへ置換する。分析条件、推薦方針、配分、承認、設定変更は通常ビューで行い、確定eventだけをOfficeのTask状態へ同期する。
