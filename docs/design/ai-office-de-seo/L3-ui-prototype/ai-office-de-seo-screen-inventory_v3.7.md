...
**現行役割（2026-08-03）**: Officeビューは監視専用ではない。通常ビューが推薦、承認、日次判断を簡単に行う面であるのに対し、Officeは同じ業務正本を市場、Share、Keyword、記事、Task、Agent、評価、学習まで深掘りし、推薦方針、配分、条件、実行順、停止・再開等の詳細を操作する面である。Agentへの会話は質問回答と変更Proposalを分け、変更は対象、差分、影響、credit、権限を確認後に共通Command／Eventへ反映する。Office独自の業務状態を作らず、通常ビューと双方向同期する。以下の2026-07-09「監視面・決める操作を持たない」という記録は旧プロト実装の履歴であり、現行要求として使用しない。既存の部屋、Agent、詳細ページ、ライブ表示、往復導線は再利用し、操作能力を現行業務Lifecycleへ追従させる。
