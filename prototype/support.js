...
(() => {
  ...
  function walkText(node) {
    ...
    // SVG内のテキストバインディングをHTML spanで包むと描画されない（SVGはspanを無視する）ため、
    // 親がSVG名前空間なら tspan で包む。※生成元dc-runtimeが手元に無いためビルド成果物を直接修正
    const interpTag = node.parentElement && node.parentElement.namespaceURI === "http://www.w3.org/2000/svg" ? "tspan" : "span";
